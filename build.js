...
const DIST_VERSION_JS = path.join(DIST_DIR, 'version.js');
...
// Writes dist/version.js — a tiny static file the in-app update
// checker fetches via cross-origin <script> injection (the only
// mechanism that works from file:// where fetch is blocked). Lives
// next to dist/taigan-bridge.html so the existing GitHub Pages
// workflow uploads it as part of the regular Pages artifact, with
// no workflow changes needed.
//
// Schema (bump SUPPORTED_SCHEMAS in update-check.js if you change
// shape incompatibly):
//   schema: int      — format version
//   stable: string   — latest stable release tag (e.g. "0.2.0")
...
function writeVersionJs(meta) {
  ...
  const js =
    ...
    ' *\n' +
    ' * Served from beichhorn-taigan.github.io/taigan-bridge/version.js\n' +
    ' * and read by update-check.js via cross-origin <script> injection.\n' +
    ' * Static — do NOT hand-edit; regenerated on every build.\n' +
    ' */\n' +
    '(function () {\n' +
    '  try {\n' +
    ...
    '  } catch (e) {}\n' +
    '})();\n';
  fs.writeFileSync(DIST_VERSION_JS, js, 'utf8');
  return js.length;
}
...
function main() {
  ...
  // Emit dist/version.js so the in-app update checker can read it
  // from the deployed GitHub Pages site. Uploaded automatically by
  // .github/workflows/deploy-pages.yml (which uploads all of dist/).
  const versionBytes = writeVersionJs(meta);
  console.log(`  version    ${path.relative(ROOT, DIST_VERSION_JS)} (${versionBytes} bytes)`);
}
