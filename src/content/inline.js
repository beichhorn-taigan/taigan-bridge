...
(function () {
  ...
  window.TB.content.onboardingQuestions = [
    ...
    },
    {
      ...
      helpText_en: 'Japanese Nationality Law Article 14 requires those with multiple citizenships to choose one by age 22. The Family module surfaces 国籍選択 deadlines based on this. You can refine per-child later in the Family page.',
      helpText_jp: '日本国籍法第 14 条は、複数の国籍を有する者は 22 歳までに一つを選択するよう求めています。Family モジュールでこの回答に基づき 国籍選択 期限を表示します。後で Family ページで個別調整も可能です。',
      options: [
        { value: 'all_under_18', label_en: 'All under 18 (deadline still distant)',         label_jp: '全員 18 歳未満(期限はまだ先)' },
        { value: 'has_18_22',    label_en: 'At least one is 18-22 (deadline within reach)', label_jp: '18〜22 歳の子あり(期限が近い)' },
        { value: 'has_over_22',  label_en: 'At least one is over 22 (overdue / chose)',     label_jp: '22 歳超の子あり(期限超過 or 選択済み)' },
    ...
    },
    {
      ...
      helpText_en: 'Medicare eligibility starts at age 65. Part A is free with 40 quarters of work history; Part B has a monthly premium ($175+ in 2026, with IRMAA surcharges based on MAGI). Living abroad doesn\'t exempt you from Part B premiums if you\'re enrolled, and the late-enrollment penalty is permanent. The Healthcare module surfaces what to do based on this.',
      helpText_jp: 'Medicare の受給開始は 65 歳。Part A は 40 単位以上の労働歴で無料、Part B は月額保険料($175+/月 2026 年)+ MAGI ベースの IRMAA 加算あり。海外居住でも加入中は Part B 保険料は支払い必要、加入遅延ペナルティは永続。Healthcare モジュールでこの回答に基づき対応を案内します。',
      options: [
